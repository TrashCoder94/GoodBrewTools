...
namespace reflect
{
	...
	struct TypeDescriptor_StdVector_Bool : TypeDescriptor {
		TypeDescriptor* itemType;
		void(*addNewItem)(const void*);
		void(*removeItem)(const void*, size_t);
		void(*clearAllItems)(const void*);
		...
		template <typename ItemType>
		TypeDescriptor_StdVector_Bool(ItemType*, FieldType vectorFieldType = FieldType::Vector)
			: TypeDescriptor{ "std::vector<>", sizeof(std::vector<ItemType>), vectorFieldType },
			itemType{ TypeResolver<ItemType>::get() } {
			addNewItem = [](const void* vecPtr) {
				auto& vec = *(std::vector<ItemType>*) vecPtr;
			...
			getItem = [](const void* vecPtr, size_t index) -> const void* {
				const auto& vec = *(const std::vector<ItemType>*) vecPtr;
				int* pBoolAsIntValue = reinterpret_cast<int*>(vec[index]);
				void* pIntAsVoidValue = reinterpret_cast<void*>(pBoolAsIntValue);
				return pIntAsVoidValue;
				};
		}
		virtual std::string getFullName() const override {
			return std::string("std::vector<") + itemType->getFullName() + ">";
	...
	template <>
	class TypeResolver<std::vector<bool>> {
	public:
		static TypeDescriptor* get() {
			static TypeDescriptor_StdVector_Bool typeDesc{ (bool*) nullptr };
			return &typeDesc;
		}
	};
}